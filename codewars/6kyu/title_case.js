/*
Solution to https://www.codewars.com/kata/title-case/
...
Second argument (optional): space-delimited list of minor words that must always be lowercase except for the first word
  in the string. The JavaScript/CoffeeScript tests will pass undefined when this argument is unused.
...
function titleCase(sentence, minor_words = "") {

  const minor_trimed = minor_words.trim();
  const sentence_trimed = sentence.trim();

  if (sentence_trimed.length === 0) {
    return "";
  }

  const minor =
    minor_trimed.length === 0
      ? []
      : minor_trimed.toLowerCase().split(" ");

  const words = sentence_trimed.toLowerCase().split(" ");

  const first = titleCaseSingleWord(words[0]);
  const rest =
    words
    .slice(1)
    .map(word => minor.includes(word) ? word : titleCaseSingleWord(word));

  return [first, ...rest].join(" ");
}

const titleCaseSingleWord = word =>
